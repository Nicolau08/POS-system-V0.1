-- Fix create_order_with_items stock_movements aggregate.
-- Error fixed: "column i.value must appear in GROUP BY ..."
...
AS $$
DECLARE
  v_jwt_tenant TEXT := NULLIF(auth.jwt() ->> 'tenant_id', '');
...
BEGIN
  IF v_is_service_role THEN
    v_tenant_id := COALESCE(v_from_json, v_jwt_tenant);
  ELSE
    v_tenant_id := v_jwt_tenant;
  END IF;
  ...
  IF jsonb_typeof(items) = 'array' AND jsonb_array_length(items) > 0 THEN
    IF EXISTS (
      ...
      LEFT JOIN public.products p
        ON p.id = NULLIF(i.value->>'product_id', '')::uuid
       AND p.tenant_id = v_tenant_id
      WHERE NULLIF(i.value->>'product_id', '') IS NOT NULL
        AND p.id IS NULL
    ) THEN
    ...
    SELECT
      v_tenant_id,
      v_order_id,
      NULLIF(i.value->>'product_id', '')::uuid,
      COALESCE(i.value->>'product_name', 'Produto'),
      COALESCE((i.value->>'quantity')::numeric, 0),
      COALESCE((i.value->>'price')::numeric, 0),
      COALESCE((i.value->>'discount_amount')::numeric, 0)
    FROM jsonb_array_elements(items) AS i(value);

    INSERT INTO public.stock_movements (product_id, type, quantity, reference_id)
    SELECT
      NULLIF(i.value->>'product_id', '')::uuid AS product_id,
      'sale'::text,
      SUM(COALESCE((i.value->>'quantity')::numeric, 0)) * -1 AS quantity,
      'order:' || v_order_id::text AS reference_id
    FROM jsonb_array_elements(items) AS i(value)
    WHERE NULLIF(i.value->>'product_id', '') IS NOT NULL
    GROUP BY NULLIF(i.value->>'product_id', '')::uuid
