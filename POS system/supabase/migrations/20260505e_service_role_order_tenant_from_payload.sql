-- Sync (Supabase service_role) must use tenant_id from order_data, not JWT claims.
-- JWT-based POS sessions keep tenant from current_tenant_id(); service_role trusts explicit row tenant.
...
AS $$
DECLARE
  v_jwt_tenant TEXT := public.current_tenant_id();
...
BEGIN
  ...
  IF v_jwt_tenant IS NULL OR btrim(v_jwt_tenant) = '' THEN
    RAISE EXCEPTION 'Missing tenant in JWT claims'
      USING ERRCODE = '22023';
  END IF;

  NEW.tenant_id := COALESCE(NULLIF(NEW.tenant_id, ''), v_jwt_tenant);
  IF NEW.tenant_id IS NULL OR btrim(NEW.tenant_id) = '' THEN
    RAISE EXCEPTION 'orders.tenant_id cannot be null/empty'
      USING ERRCODE = '23502';
  END IF;

  IF NEW.tenant_id <> v_jwt_tenant THEN
...
DECLARE
  ...
  v_from_json TEXT := NULLIF(btrim(COALESCE(order_data->>'tenant_id', '')), '');
  v_tenant_id TEXT;
...
BEGIN
  IF v_role = 'service_role' THEN
    v_tenant_id := COALESCE(v_from_json, NULLIF(btrim(COALESCE(public.current_tenant_id(), '')), ''));
  ELSE
    v_tenant_id := public.current_tenant_id();
    IF (v_tenant_id IS NULL OR btrim(v_tenant_id) = '') AND v_from_json IS NOT NULL THEN
      v_tenant_id := v_from_json;
    END IF;
  END IF;

  IF v_tenant_id IS NULL OR btrim(v_tenant_id) = '' THEN
    RAISE EXCEPTION 'tenant_id is required (service_role: set order_data.tenant_id; app: JWT tenant_id claim)'
      USING ERRCODE = '22023';
  END IF;
