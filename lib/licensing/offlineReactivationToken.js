...
export function verifyOfflineReactivationToken(tokenDigits, input) {
  ...
  if (!Number.isFinite(expMs) || Date.now() > expMs) {
    return { ok: false, error: 'Token expirado. Gere outro na consola de licenças.' };
  }

  // Token já aplicado: a licença local já tem esta (ou maior) validade.
  const currentRaw = String(
    input?.currentExpirationIso ?? input?.currentExpiration ?? input?.localExpirationIso ?? '',
  ).trim();
  const currentMs = Date.parse(currentRaw);
  if (Number.isFinite(currentMs) && currentMs >= expMs - 60_000) {
    return {
      ok: false,
      error: 'Este token já foi utilizado. Peça um novo na consola.',
    };
  }
