...
export function signCanonicalLicensePayload(canonicalPayload, secret) {
  return crypto.createHmac('sha256', secret).update(JSON.stringify(canonicalPayload)).digest('hex');
}
...
export function encodeLicenseBase64(license) {
  return Buffer.from(JSON.stringify(license), 'utf8').toString('base64');
}

export const ACTIVATION_VOUCHER_KIND = 'pos_activation_v1';
...
export function signCanonicalVoucherPayload(canonical, secret) {
...
export function verifyActivationVoucher(payload, secret) {
...
export function encodeVoucherBase64(voucher) {
  return Buffer.from(JSON.stringify(voucher), 'utf8').toString('base64');
}

export function materializeMachineLicenseFromVoucher(voucher, machineId, secret) {
...
export function verifySignedMachineLicense(payload, secret) {
  if (!payload || typeof payload !== 'object') {
...
export function parseJsonOrBase64License(rawInput) {
