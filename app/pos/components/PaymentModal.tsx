...
import { formatPaymentMethodLabel, getPaymentMethodColor } from '@/lib/paymentMethodLabel';
...
export function PaymentModal({
  ...
  useEnterToConfirm(isOpen && canFinalize && !isFinalizing, onFinalize, onClose);
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center p-4 pos-modal-overlay backdrop-blur-md"
        ...
        >
          <motion.div
            ...
            className="bg-pos-surface border border-pos-border rounded w-full max-w-[450px] overflow-hidden flex flex-col max-h-[90vh]"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="p-4 border-b border-pos-border bg-pos-card">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="text-xl font-bold text-pos-fg">{modalTitle}</h3>
                  <div className="flex gap-4 mt-2 text-xs text-pos-muted">
                ...
                {!hideReceiptPrint ? (
                  <button
                    ...
                    className={`w-14 h-12 rounded border flex items-center justify-center transition-colors ${
                      isReceiptPrintEnabled
                        ? 'bg-pos-action border-pos-border text-pos-fg hover:bg-pos-action-hover'
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              <div className="text-sm font-medium text-pos-muted capitalize">Resumo do pedido</div>
              <div className="space-y-2 text-pos-fg">
                {cart.map((item) => (
              ...
              <div className="pt-3 border-t border-pos-border space-y-1 text-pos-muted text-sm">
              ...
              <div className="pt-3 border-t border-pos-border flex justify-between items-baseline">
                <span className="text-4xl font-bold text-pos-fg">Total</span>
                <span className="text-4xl font-bold tabular-nums text-[#15803d]">{formatPrice(total)}</span>
              </div>
              ...
              <div className="pt-4 space-y-3">
                ...
                <div className="flex items-center justify-between gap-2">
                  <span className="text-sm font-medium text-pos-muted capitalize">Método de pagamento</span>
                    ...
                    {allowPartialPayment ? (
                      <button
                        ...
                        className={`h-9 px-4 rounded font-bold text-xs transition-colors ${
                          isPartialPayment
                            ? 'pos-btn-brand'
                            : 'bg-pos-action border border-pos-border text-pos-fg hover:bg-pos-action-hover'
                        }`}
                      >
                        Parcial
                      </button>
                ...
                {partialModeActive && setPaymentAmount ? (
                  <div className="space-y-2 rounded border border-pos-border bg-zinc-900/40 p-3">
                    <label className="block space-y-1.5">
                      ...
                      <input
                        ...
                        placeholder="0"
                        autoFocus
                        className="w-full h-11 px-3 bg-zinc-950 border border-pos-border rounded text-right text-[#a5b4fc] font-bold outline-none focus:border-[#0001fb] placeholder:text-zinc-600"
                      />
                    </label>
                    {Number.isFinite(paymentAmountParsed) && paymentAmountParsed > 0 ? (
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {enabledMethods.map((method) => (
                      <PaymentMethodButton
                        ...
                        color={method.color || getPaymentMethodColor(method.code || method.name)}
                      />
                    ))}
                  </div>
                ) : !isProforma ? (
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        placeholder="Valor"
                        className="flex-1 h-11 px-3 bg-pos-field border border-pos-border rounded text-right text-pos-fg font-bold outline-none focus:border-[#0001fb] placeholder:text-pos-muted"
                      />
                      <button
                        onClick={() => {
                        ...
                        className="h-11 px-4 bg-pos-action hover:bg-pos-action-hover border border-pos-border text-pos-fg rounded font-bold text-sm"
                      >
                        Adicionar
                      </button>
                    </div>
                    ...
                    {payments.length > 0 && (
                      <div className="space-y-1 text-sm border border-pos-border rounded p-3 bg-zinc-900/40">
                        {payments.map((p, i) => (
                          <div key={`${p.method}-${p.amount}-${i}`} className="flex items-center justify-between gap-2 text-zinc-300">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                ...
                                className="h-7 w-7 rounded border border-pos-border text-zinc-400 hover:text-red-400 hover:border-red-500 hover:bg-red-500/10 transition-colors flex items-center justify-center"
                              >
                                <X size={14} />
                              </button>
                            </div>
                        ...
                        <div className="pt-2 mt-2 border-t border-pos-border flex justify-between text-zinc-400">
                          <span>Falta</span>
                ...
                {!isProforma && !isMultiplePayment && isCashMethod(paymentMethod, paymentMethods) && (
                  <div className="space-y-2 border border-pos-border rounded p-3 bg-pos-card">
                    <input
                      type="text"
                      inputMode="decimal"
                      ...
                      className="w-full h-11 px-3 bg-pos-field border border-pos-border rounded text-right text-pos-fg font-bold outline-none focus:border-[#0001fb] placeholder:text-pos-muted"
                    />
                    <div className="flex justify-between text-pos-fg text-sm">
                      <span>Troco:</span>
                      <span className="font-bold tabular-nums text-[#15803d]">{formatPrice(singleChange)}</span>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="p-4 flex gap-3">
              <button onClick={onClose} className="flex-1 h-12 bg-pos-action hover:bg-pos-action-hover border border-pos-border text-pos-fg text-sm rounded font-medium transition-all">
                Voltar
              </button>
              ...
              <button
                ...
                className={`flex-1 h-12 rounded text-sm font-medium transition-all flex items-center justify-center gap-2 ${
                  canFinalize && !isFinalizing
                    ? 'pos-btn-brand'
                    : 'bg-pos-action text-pos-muted border border-pos-border cursor-not-allowed'
                }`}
              >
