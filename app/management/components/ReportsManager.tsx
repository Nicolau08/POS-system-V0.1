      ...
      <head>
        ...
        <style>
          * { box-sizing: border-box; }
          ...
          .report-print-root { width: 210mm; min-height: 297mm; margin: 0 auto; padding: 14mm 12mm 12mm; background: white; }
          table { width: 100%; border-collapse: collapse; font-size: 11px; }
          th { background: #d9d3b4; padding: 8px 10px; text-align: left; border-bottom: 1px solid #ddd; }
          td { padding: 7px 10px; border-bottom: 1px solid #ececec; }
...
export default function ReportsManager() {
  ...
  const applyPresetPeriod = (preset: 'today' | 'yesterday' | 'thisWeek' | 'lastWeek' | 'thisMonth' | 'lastMonth' | 'thisYear' | 'lastYear') => {
    ...
    setTempDateFrom(toInputDate(start));
    setTempDateTo(toInputDate(end));
  };
  ...
  return (
    <>
      <div className="flex h-full bg-[#0a0a0a] text-zinc-200 overflow-hidden">
        <div className="flex-1 min-w-0 border-r border-zinc-800/50 flex flex-col bg-[#0a0a0a]">
          <div className="px-4 pt-4">
            <div className="flex items-center gap-2 px-3 py-2 bg-[#131314] border border-zinc-700 rounded max-w-sm hover:border-zinc-600 transition-colors">
        ...
        <aside className="w-[340px] shrink-0 bg-[#0a0a0a] flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 custom-scrollbar">
            ...
            <div>
              ...
              >
                ...
                <span className="text-sm">{formatDate(dateFrom)} - {formatDate(dateTo)}</span>
              </button>
            </div>
      ...
      {isPreviewOpen && builtReport && (
        ...
        >
          <div
            className="w-full max-w-[1200px] max-h-[92vh] bg-[#111] border border-zinc-800 rounded shadow-2xl overflow-hidden"
          ...
          >
            <div className="h-14 border-b border-zinc-800 px-4 flex items-center justify-between bg-[#141414]">
              <div>
                ...
                <p className="text-xs text-zinc-500">Pré-visualização em A4</p>
              </div>
            ...
            <div className="max-h-[calc(92vh-56px)] overflow-auto custom-scrollbar bg-[#1a1a1a] p-6">
              <div
                ...
                style={{ width: '210mm', minHeight: '297mm', padding: '14mm 12mm 12mm' }}
              >
                <div className="flex items-start justify-between mb-5">
                  ...
                  <div className="text-[12px] text-zinc-600">{formatDate(new Date().toISOString())}</div>
                </div>

                <div className="grid grid-cols-2 gap-x-10 gap-y-2 mb-5 text-[12px]">
                ...
                <div className="grid grid-cols-3 gap-3 mb-5">
                  {builtReport.summaries.map((item) => (
                    <div key={item.label} className="border border-zinc-200 bg-zinc-50 px-3 py-2">
                      <div className="text-[11px] uppercase tracking-[0.12em] text-zinc-500">{item.label}</div>
                      <div className="text-[16px] font-semibold mt-1">{item.value}</div>
                    </div>
                  ))}
                </div>
                ...
                <div className="border border-zinc-200 overflow-hidden">
                  <table className="w-full border-collapse text-[11px]">
                    <thead>
                      <tr className="bg-[#d2d0bb]">
                        {builtReport.columns.map((column) => (
                          <th key={column} className="px-3 py-2 text-left font-semibold border-b border-zinc-200">
                            {column}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody>
                      {builtReport.rows.length > 0 ? (
                        builtReport.rows.map((row, index) => (
                          <tr key={`${builtReport.key}-${index}`} className={index % 2 === 0 ? 'bg-white' : 'bg-zinc-50'}>
                            {builtReport.columns.map((column) => (
                              <td key={column} className="px-3 py-2 border-b border-zinc-100 align-top">
                                {String(row[column] ?? '-')}
                              </td>
                            ))}
                          </tr>
                ...
                <div className="mt-6 flex items-center justify-between text-[11px] text-zinc-600">
                  <span>NINO POS • Relatórios</span>
                  <span>Registos: {builtReport.rows.length}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {isPeriodModalOpen && (
        ...
        >
          <div
            ...
            onClick={(event) => event.stopPropagation()}
          >
            <div className="px-6 py-5 border-b border-zinc-700 text-center">
              <h3 className="text-[18px] text-white">Período</h3>
              <div className="mt-4 inline-flex items-center rounded-md bg-[#2a9cd4] px-4 py-2 text-white font-bold">
            ...
            <div className="grid grid-cols-1 md:grid-cols-[1fr_1fr_280px] gap-6 p-6">
              <div>
                ...
                <div className="bg-[#131314] border border-zinc-700 rounded p-3">
              ...
              <div>
                <p className="text-sm text-zinc-100 mb-3">Período pré-definido</p>
...
function PresetButton({
  ...
  return (
    <button
      onClick={onClick}
      className="min-h-11 px-3 py-3 border border-zinc-700 rounded bg-[#131314] text-white text-sm hover:bg-zinc-800 hover:border-zinc-600 transition-colors"
    >
      {label}
    </button>
  );
