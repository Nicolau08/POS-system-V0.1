  ...
  if (serialCandidate) {
    ...
    if (parsed.error || !parsed.payload) {
      return {
        error:
          parsed.error ||
          'Informe o número de série (ex.: D_9L6WAKYU) ou uma chave de licença JSON/base64 assinada.',
        status: 400,
      };
    }
    parsedLicense = parsed;
    const validated = validateMachineBoundLicense(parsedLicense.payload, status.tenantId || null);
    if (!validated.ok) {
    ...
    JSON.stringify(
      {
        ...
        licenseMode: serialMode ? 'serial' : 'signed',
      },
      null,
      2
    ),
