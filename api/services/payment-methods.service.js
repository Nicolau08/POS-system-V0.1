...
export async function ensureEssentialPaymentMethods(tenantId) {
  ...
  await run(
    `UPDATE payment_methods SET enabled = 0, updated_at = ?
     WHERE tenant_id = ? AND LOWER(code) IN ('card', 'pix')`,
    [now, tenantId],
  );
  ...
  for (const spec of DEFAULT_PAYMENT_METHOD_SPECS) {
    const existing = await get(
      `SELECT id FROM payment_methods WHERE tenant_id = ? AND LOWER(code) = LOWER(?) LIMIT 1`,
      [tenantId, spec.code],
    );
    if (existing?.id) continue;
    const [row] = buildDefaultPaymentMethodInsertRows(tenantId, now).filter(
      (r) => String(r[1]).toLowerCase() === spec.code,
    );
    if (row) {
